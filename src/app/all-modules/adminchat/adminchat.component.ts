...
export class AdminchatComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    if ($('#demoSettings').length === 0) {
      $('.main-wrapper').append(
        '<div class="skin-settings" id="demoSettings">' +
          '<div class="skin-sett-icon"><i class="fa fa-cog"></i></div>' +
          '<div class="skin-sett-body">' +
          '<h4>Template Colors</h4>' +
          '<ul class="skin-colors">' +
          '<li><a class="skin-purple" data-color="default" href="javascript:void(0);"></a></li>' +
          '<li><a class="skin-red" data-color="red" href="javascript:void(0);"></a></li>' +
          '<li><a class="skin-teal" data-color="teal" href="javascript:void(0);"></a></li>' +
          '<li><a class="skin-orange" data-color="orange" href="javascript:void(0);"></a></li>' +
          '</ul>' +
          '</div>' +
          '</div>'
      );
    ...
    if (!!hasTemp) {
      $('head').append(
        '<link id="tempSkin" rel="stylesheet" href="assets/css/style-' +
          hasTemp +
          '.css">'
      );
      $('body')
    ...
    if (this.functionInvoke.subsVar == undefined) {
      this.functionInvoke.subsVar = this.functionInvoke.invokeFirstComponentFunction.subscribe(
        (name: string) => {
          this.firstFunction();
        }
      );
      this.functionInvoke.subsVar = this.functionInvoke.invokeLoginComponentFunction.subscribe(
        (name: string) => {
          this.LoginFunction();
        }
      );
      this.functionInvoke.subsVar = this.functionInvoke.invoketoggle.subscribe(
        (name: string) => {
          this.bodyClassAdd();
        }
      );
    }
    // if (this.url1 === 'dashboard') {
    //   this.show = true;
    // }
    if (this.url1 === 'error') {
      this.show = false;
    }
    if (this.url1 === 'forgotpassword') {
      this.show = false;
    }
    if (this.url1 === 'login') {
      this.show = false;
    }
    if (this.url1 === 'ComingSoon') {
      this.show = false;
    }
    // if (this.url1 === 'dashboard') {
    //   this.show = true;
    // }
    // if (this.url1 === 'components') {
    //   this.show = false;
    // }
    // if (this.url1 === 'login') {
    //   this.show = false;
  ...
  getClass() {
    var classList = '';
    if (this.url1 === 'login') {
      classList = 'main-wrapper login-body';
    } else if (this.url1 === 'forgotpassword') {
      classList = 'main-wrapper login-body';
    } else {
  ...
  bodyClassAdd() {
    ...
    if (this.showMinibar === true) {
      this.renderer.addClass(document.body, 'mini-sidebar');
    }
    if (this.showMinibar === false) {
